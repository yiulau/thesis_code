...
def NUTS(q_init,epsilon,pi,leapfrog,NUTS_criterion):
    lam,Q = eigen(getH(q_init,V).data)
    p = Variable(generate_momentum(alp,lam,Q),requires_grad=False)
    ...
    p_sleft = Variable(p_sharp(q,p).clone(),requires_grad=False)
    p_sright = Variable(p_sharp(q,p).clone(),requires_grad=False)
    j = 0
