...
def leapfrog_window(q,p,epsilon,pi,logw_old,qprop_old,pprop_old):
    p_prime = Variable(p.data.clone(),requires_grad=False)
    q_prime = Variable(q.data.clone(),requires_grad=True)
    #print(q_prime.data,p_prime.data)
    H = pi(q_prime,p_prime)
    ...
    return(q_prime,p_prime,qprop,pprop,logw_prop)

def HMC_alt_windowed(epsilon, L, current_q, leapfrog, pi):
    ...
    p_prop = p.clone()
    for _ in range(L):
        o = leapfrog_window(q, p, epsilon, pi,logw_prop,q_prop,p_prop)
...
#out = HMC_alt_windowed(0.1,10,q,leapfrog,pi)
#print(out)
#exit()
